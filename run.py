...
twitch_miner = TwitchChannelPointsMiner(
    username="doicdj",
    password="Shadow112",           # If no password will be provided, the script will ask interactively
    ...
    logger_settings=LoggerSettings(
        ...
        discord=Discord(
            webhook_api="https://discord.com/api/webhooks/1278001551188689041/CKzFROsySbZcPaF3qvbvJ_QfEXLCKaUrpJK-rvUVzumN1Oq-4CUTuLAyKAKW6Cu6OiSB",  # Discord Webhook URL
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE,
                    Events.BET_LOSE, Events.CHAT_MENTION,
                   Events.GAIN_FOR_WATCH, Events.GAIN_FOR_CLAIM],                                  # Only these events will be sent to the chat
        ),
        webhook=Webhook(
...
twitch_miner.mine(
    [
        ...
        Streamer("playoverwatch", settings=StreamerSettings(make_predictions=True  , follow_raid=False ,                     watch_streak=True , bet=BetSettings(strategy=Strategy.SMART      , percentage=5 , stealth_mode=False, percentage_gap=30 , max_points=50000 , filter_condition=FilterCondition(by=OutcomeKeys.ODDS,             where=Condition.LT,  value=300 ) ) )),
        Streamer("kitsfu", settings=StreamerSettings(make_predictions=False , follow_raid=True  ,                     watch_streak=True                                                                                                                                                                                                                                 )),
